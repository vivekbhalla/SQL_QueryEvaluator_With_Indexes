...
public class OperatorGroupBy {
	...
	public LinkedHashMap<Integer, List<Tuple>> calculateSingleGroupBy() {

		LinkedHashMap<String, LinkedList<List<Tuple>>> grouped = new LinkedHashMap<String, LinkedList<List<Tuple>>>();
		int[] cntList = new int[col.size()];
		int c = 0;
		for (Column colName : col) {
			cntList[c++] = ConditionEvaluator.getColNumber(colName
					.getColumnName(), OutputSchema.osTable.getWholeTableName()
					.toLowerCase());
		}
		...
		for (int k : instream.keySet()) {
			StringBuilder key = new StringBuilder();
			for (int cnt : cntList) {
				key.append(instream.get(k).get(0).getTupleVaule()[cnt]);
